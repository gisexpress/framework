...
namespace System.Geometries.Algorithm
{
    ...
    internal abstract class LineIntersector 
    {
        ...
        public static double ComputeEdgeDistance(ICoordinate p, ICoordinate p0, ICoordinate p1)
        {
            ...
            if (p.IsEquivalent(p0))
            {
                dist = 0.0;
            }
            else if (p.IsEquivalent(p1))
            {
                dist = dx > dy ? dx : dy;
            }
            else
            {
                ...
                if (dist == 0.0 && !p.IsEquivalent(p0))
                    dist = Math.Max(pdx, pdy);

            }
            
            Debug.Assert(!(dist == 0.0 && ! p.IsEquivalent(p0)), "Bad distance calculation");
            return dist;
        }
        ...
        public static double NonRobustComputeEdgeDistance(ICoordinate p, ICoordinate p1, ICoordinate p2)
        {
            ...
            Debug.Assert(! (dist == 0.0 && ! p.IsEquivalent(p1)), "Invalid distance calculation");
            return dist;
        }
        ...
        protected ICoordinate[] IntersectionPoint = new ICoordinate[2];
        ...
        /// </summary>
        protected ICoordinate Pa { get { return IntersectionPoint[0]; } }

        /// <summary>
        /// Alias the <see cref="IntersectionPoint"/>[1] for ease of reference
        /// </summary>
        protected ICoordinate Pb { get { return IntersectionPoint[1]; } }
        ...
        public bool IsIntersection(ICoordinate pt) 
        {
            for (var i = 0; i < Result; i++) 
                if (IntersectionPoint[i].IsEquivalent(pt)) 
                    return true;                        
            return false;
        }
        ...
        public bool IsInteriorIntersection(int inputLineIndex)
        {
            for (var i = 0; i < Result; i++)
            {
                if (!(IntersectionPoint[i].IsEquivalent(InputLines[inputLineIndex][0]) ||
                      IntersectionPoint[i].IsEquivalent(InputLines[inputLineIndex][1])))                                   
                    return true;
            }
            return false;
        }
