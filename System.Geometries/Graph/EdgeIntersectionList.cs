...
namespace System.Geometries.Graph
{
    ...
    internal class EdgeIntersectionList
    {
        ...
        readonly IDictionary<EdgeIntersection, EdgeIntersection> nodeMap = new SortedDictionary<EdgeIntersection, EdgeIntersection>();
        readonly Edge edge;  // the parent edge
        ...
        public bool IsIntersection(ICoordinate pt)
        {
            ...
            {
                if (ei.Coordinate.IsEquivalent(pt))
                    return true;
            }
            return false;
        }
        ...
        {
            int maxSegIndex = edge.NumPoints - 1;
            Add(edge.Sequence.Get(0), 0, 0.0);
            Add(edge.Sequence.Get(maxSegIndex), maxSegIndex, 0.0);
        }
        ...
        public Edge CreateSplitEdge(EdgeIntersection ei0, EdgeIntersection ei1)
        {
            ...
            ICoordinate lastSegStartPt = edge.Sequence.Get(ei1.SegmentIndex);
            // if the last intersection point is not equal to the its segment start pt,
            // add it to the points list as well.
            // (This check is needed because the distance metric is not totally reliable!)
            // The check for point equality is 2D only - Z values are ignored
            bool useIntPt1 = ei1.Distance > 0.0 || !ei1.Coordinate.IsEquivalent(lastSegStartPt);
            if (!useIntPt1)
                npts--;

            ICoordinateCollection pts = edge.Sequence.Factory.Create<ICoordinateCollection>();

            pts.Add(new Coordinate(ei0.Coordinate));

            for (int i = ei0.SegmentIndex + 1; i <= ei1.SegmentIndex; i++)
                pts.Add(edge.Sequence.Get(i));

            if (useIntPt1)
                pts.Add(ei1.Coordinate);
