...
namespace System.Geometries.Index.Quadtree
{
    ...
    internal class Root<T> : NodeBase<T>
    {
        ...
        public void Insert(IEnvelope itemEnv, T item)
        {
            ...
            */
            if (node == null || ! node.Bounds.Contains(itemEnv)) 
            {
        ...
        {
            Debug.Assert(tree.Bounds.Contains(itemEnv));
            /*
            * Do NOT create a new quad for zero-area envelopes - this would lead
            * to infinite recursion. Instead, use a heuristic of simply returning
            * the smallest existing quad containing the query
            */
            bool isZeroX = IntervalSize.IsZeroWidth(itemEnv.Min.X, itemEnv.Max.X);
            bool isZeroY = IntervalSize.IsZeroWidth(itemEnv.Min.Y, itemEnv.Max.Y);
            NodeBase<T> node;
            if (isZeroX || isZeroY)
