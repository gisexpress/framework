...
namespace System.Geometries.Index.Strtree
{
    ...
    public abstract class AbstractSTRtree<T, TItem> where T : IIntersectable<T>, IExpandable<T>
    {
        ...
        {
            Debug.Assert(!(_built || _building), "Cannot insert items into an STR packed R-tree after it has been built.");
        ...
        private void Query(T searchBounds, AbstractNode<T, TItem> node, IList<TItem> matches)
        {
            foreach (var childBoundable in node.ChildBoundables)
            {
                ...
                else Debug.Fail("ShouldNeverReachHere");
            }
        }
        ...
        private static void BoundablesAtLevel(int level, AbstractNode<T, TItem> top, ICollection<IBoundable<T, TItem>> boundables)
        {
            ...
            foreach (var boundable in top.ChildBoundables)
            {
                ...
                {
                    Debug.Assert(boundable is ItemBoundable<T, TItem>);
                    if (level == -1)
