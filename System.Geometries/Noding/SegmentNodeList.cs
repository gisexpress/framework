...
{
    internal class SegmentNodeList : IEnumerable<object>
    {
        ...
        public SegmentNode Add(ICoordinate intPt, int segmentIndex)
        {
            ...
            if (_nodeMap.TryGetValue(eiNew, out eiObj))
            {
                ...
                Debug.Assert(ei.Coord.IsEquivalent(intPt), "Found equal nodes with different coordinates");
                return ei;
            }
        ...
        {
            var maxSegIndex = _edge.Sequence.Count - 1;

            Add(_edge.Sequence.Get(0), 0);
            Add(_edge.Sequence.Get(maxSegIndex), maxSegIndex);
        }
        ...
        private void AddCollapsedNodes()
        {
            ...
            foreach (var vertexIndex in collapsedVertexIndexes)
                Add(_edge.Sequence.Get(vertexIndex), vertexIndex);
        }
        ...
        {
            for (var i = 0; i < _edge.Sequence.Count - 2; i++)
            {
                var p0 = _edge.Sequence.Get(i);
                var p2 = _edge.Sequence.Get(i + 2);

                if (p0.IsEquivalent(p2))
                {
        ...
        private static bool FindCollapseIndex(SegmentNode ei0, SegmentNode ei1, int[] collapsedVertexIndex)
        {
            ...
            if (!ei0.Coord.IsEquivalent(ei1.Coord))
                return false;
        ...
        */
        ...
        ISegmentString CreateSplitEdge(SegmentNode ei0, SegmentNode ei1)
        {
            ...
            var lastSegStartPt = _edge.Sequence.Get(ei1.SegmentIndex);
            ...
            var useIntPt1 = ei1.IsInterior || !ei1.Coord.IsEquivalent(lastSegStartPt);
            ...
            ICoordinateCollection pts = _edge.Sequence.Factory.Create<ICoordinateCollection>();

            pts.Add(new Coordinate(ei0.Coord));

            for (var i = ei0.SegmentIndex + 1; i <= ei1.SegmentIndex; i++)
            {
                pts.Add(_edge.Sequence.Get(i));
            }

            if (useIntPt1)
            {
                pts.Add(new Coordinate(ei1.Coord));
            }
        ...
        public ICoordinate[] GetSplitCoordinates()
        {
            ...
            CoordinateList coordList)
        {
            ...
            for (var i = ei0.SegmentIndex + 1; i <= ei1.SegmentIndex; i++)
            {
                coordList.Add(_edge.Sequence.Get(i));
            }
