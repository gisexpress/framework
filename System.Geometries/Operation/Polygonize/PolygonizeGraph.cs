...
namespace System.Geometries.Operation.Polygonize
{
    ...
    internal class PolygonizeGraph : PlanarGraph
    {
        ...
        static int GetDegreeNonDeleted(Node node)
        {
            int degree = 0;
        ...
        static int GetDegree(Node node, long label)
        {
            IList<DirectedEdge> edges = node.OutEdges.Edges;
            int degree = 0;
            foreach (PolygonizeDirectedEdge de in edges)
        ...
        public static void DeleteAllEdges(Node node)
        {
            ...
            {
                de.Marked = true;
                PolygonizeDirectedEdge sym = (PolygonizeDirectedEdge)de.SymmetricEdge;
        ...
        public void AddEdge(ILineString line)
        {
            ...
            ICoordinate[] linePts = line.Coordinates.ToArray();
            if (linePts.Length < 2)
                return; // see #87
        ...
        static void ConvertMaximalToMinimalEdgeRings(IEnumerable<DirectedEdge> ringEdges)
        {
        ...
        /// </returns>
        static IEnumerable<Node> FindIntersectionNodes(PolygonizeDirectedEdge startDE, long label)
        {
            ...
            do
            {
                ...
                Debug.Assert(de == startDE || !de.IsInRing, "found DE already in ring");
            }
            while (de != startDE);
            return intNodes;
        }
        ...
        static IList<DirectedEdge> FindLabeledEdgeRings(IEnumerable<DirectedEdge> dirEdges)
        {
        ...
        static void Label(IEnumerable<DirectedEdge> dirEdges, long label)
        {
            foreach (PolygonizeDirectedEdge de in dirEdges)
                de.Label = label;
        }

        static void ComputeNextCWEdges(Node node)
        {
            ...
            foreach (PolygonizeDirectedEdge outDE in deStar.Edges)
            {
                ...
                if (prevDE != null)
                {
                    PolygonizeDirectedEdge sym = (PolygonizeDirectedEdge)prevDE.SymmetricEdge;
                    sym.Next = outDE;
                }
                prevDE = outDE;
            }
        ...
        /// <param name="node"></param>
        /// <param name="label"></param>
        static void ComputeNextCCWEdges(Node node, long label)
        {
            DirectedEdgeStar deStar = node.OutEdges;
        ...
        ///</summary>
        /// <param name="de"></param>
        void ComputeDepthParity(PolygonizeDirectedEdge de)
        {
