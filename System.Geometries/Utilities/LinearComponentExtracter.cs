...
namespace System.Geometries.Utilities
{
    ...
    public class LinearComponentExtracter : IGeometryComponentFilter
    {
        ...
        public static IGeometry GetGeometry(IGeometry g)
        {
            return g.Factory.BuildGeometry(GetLines(g));
        }
        ...
        /// <param name="forceToLineString"><c>true</c> if <see cref="ILinearRing"/>s should be converted to <see cref="ILineString"/>s</param>
        /// <returns>A linear geometry</returns>                
        public static IGeometry GetGeometry(IGeometry g, bool forceToLineString)
        {
            return g.Factory.BuildGeometry(GetLines(g, forceToLineString));
        }
        ...
        {
            if (IsForcedToLineString && g is ILinearRing)
            {
                ILineString line = g.Factory.Create<ILineString>(g.Coordinates);
                _lines.Add(line);
                return;
            }
